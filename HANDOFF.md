...
- **Tests:** `cd web && npm test` → **355** passing (was 315; +40 from
  the pension port). `cd sidecar && npm test` → **55** passing. Both
  typechecks clean.
- **Most recent commit (this branch):** `59841c2 web: clarify pollRun
  batching + tighten interactive-modal tests`. The pension-port branch
  is committed locally, NOT pushed and NOT merged — awaiting the user's
  merge decision (PROJECT-RULES §3 session-end flow).
